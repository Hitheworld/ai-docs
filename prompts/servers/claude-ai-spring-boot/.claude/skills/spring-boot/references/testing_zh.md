...
| `@DataJpaTest` | 使用内存数据库测试 JPA 存储库 |
| `@MockBean` | 向 Spring 上下文添加模拟 bean |
| `@WithMockUser` | 用于安全测试的模拟已认证用户 |
| `@Testcontainers` | 启用 Testcontainers 支持 |
| `@ActiveProfiles` | 为测试激活特定的 Spring 配置 |

## 测试最佳实践

- 按照 AAA 模式（安排、执行、断言）编写测试
- 使用带有 @DisplayName 的描述性测试名称
- 模拟外部依赖项，使用 Testcontainers 的真实数据库
- 代码覆盖率达到 85% 以上
- 测试正常路径和边界情况
